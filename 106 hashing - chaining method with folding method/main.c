...
int hashFunction(int key) {
    int hash = 0;
    while (key > 0) {
        hash += key % 100; // Add the last two digits of the key
        key /= 100; // Remove the last two digits from the key
    }
    return hash % TABLE_SIZE; // Modulo operation to fit the table size
}

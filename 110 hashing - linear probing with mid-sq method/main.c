...
int main() {
    ...
    insert(hashTable, 654);
    insert(hashTable, 111);
    insert(hashTable, 222);
    insert(hashTable, 333);
...
int hashFunction(int key) {
    int square = key * key;
    int mid = (square / 10) % 100; // Extracting the middle part (for a 6-digit number)
    return mid % TABLE_SIZE; // Modulo operation to fit the table size
}

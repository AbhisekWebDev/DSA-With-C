...
// Top-down deletion
struct Node* deleteTopDown(struct Node* root, int key) {
...
// Main function to demonstrate the Splay Tree with top-down deletion
int main() {
    ...
    // Delete nodes using top-down approach
    root = deleteTopDown(root, 25);
    root = deleteTopDown(root, 30);

    printf("In-order traversal after top-down deletions: ");
    inorderTraversal(root);
    printf("\n");
